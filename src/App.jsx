...
function App() {
  ...
  const results = query ? fuse.search(query).map(({ item }) => item) : facultyData;
  const totalItems = results.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  const paginatedData = results.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 py-10 transition-colors duration-300">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-4 md:mb-0">Faculty Directory</h1>
